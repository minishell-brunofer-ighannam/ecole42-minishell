...
/*   Updated: 2025/12/14 19:59:58 by ighannam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	test1(void)
{
	...
	int						i;
	int						right_tokens;
	char					*expected_array[] = {"$USER", "$PLACE", "$TOWN",
							"$SATE", "$COUNTRY", NULL};
	int						expected_coords[][2] = {{14, 18}, {45, 50}, {56,
								60}, {61, 65}, {66, 73}};

	test.teste_number = 1;
	test.test_input = "hello\"my dear $USER\"'its good'\"to see you at $PLACE,
		in\"$TOWN$SATE$COUNTRY";
	...
	{
		if (!ft_strncmp(expected_array[i], splitted[i],
				ft_strlen(expected_array[i]) + 1))
			right_tokens++;
		i++;
	}
	i = 0;
	while (expected_array[i])
	{
		if (expected_coords[i][0] == coords[i][0]
			&& expected_coords[i][1] == coords[i][1])
			right_tokens++;
		i++;
	}

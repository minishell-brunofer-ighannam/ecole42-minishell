...
/*   Updated: 2025/11/26 14:06:10 by ighannam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../executer.h"

static void	ft_update_ht(t_node *node, char *pwd, char *old_pwd);
...
static void	ft_update_ht(t_node *node, char *pwd, char *old_pwd)
{
	...
	item_list_pwd = (t_linkedlist_node *)ft_find_ht(node->ht_env, "PWD");
	key_value_pwd = ft_strjoin("PWD=", pwd);
	if (item_list_pwd)
		ft_set(node->ht_env, key_value_pwd);
	item_list_old_pwd = (t_linkedlist_node *)ft_find_ht(node->ht_env, "OLDPWD");
	key_value_old_pwd = ft_strjoin("OLDPWD=", old_pwd);
	if (item_list_old_pwd)
		ft_set(node->ht_env, key_value_old_pwd);
	free(pwd);
	free(old_pwd);
	free(key_value_pwd);

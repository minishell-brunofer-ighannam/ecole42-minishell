...
O módulo **lexer** transforma a linha bruta do usuário em uma sequência estruturada de **tokens**, cada um contendo seu valor, tipo, coordenadas e callbacks de expansão.
É o elo entre o *splitter* (que apenas divide a linha) e o *expander* (que interpreta `$VAR`, globs e aspas).

O objetivo do lexer é:
...
4. **Fornecer um objeto final (`t_lexer`)**, pronto para a fase de *expansão* e, depois, *execução*.
...
3. O `lexer` recebe esses chunks e, para cada um:
...
4. Todos os tokens são guardados em um `t_lexer`, que contém:
- o vetor de tokens,
- a linha original,
- o tamanho total,
...
## **ft_lexer(prompt, expand_var, expand_glob)**

Função de entrada.
Une o splitter e o lexer:
...
## **ft_create_lexer(prompt, &splited, callbacks)**
...
É o coração da construção do lexer.

---

## **ft_destroy_lexer(self_ref)**

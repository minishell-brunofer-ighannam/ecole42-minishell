...
/*   Updated: 2025/11/11 22:06:41 by valero           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	validate_parens(const char *line)
{
	...
	while (line[++i])
	{
		if (open_parens_index > -1)
		{
			...
			else if ((i && ft_is_special_char(line, i, "(") && line[i - 1] != '$')
				|| ft_is_special_char(line, i, "("))
			{
		...
		if (ft_is_special_char(line, i, "()"))
		{
			...
			if ((!i && line[i] == '(') || (i && line[i - 1] != '$' && line[i] == '('))
			{
				open_parens_index = i;
				is_dollar_parens = false;
			}
			if (!is_dollar_parens && open_parens_index == -1 && line[i] == ')')
				return (i);
			if (open_parens_index && line[i] == ')')
				open_parens_index = -1;
		}
	}
	if (open_parens_index == -1)
